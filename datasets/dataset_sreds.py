...
class sreds_test(torch.utils.data.Dataset):
    ...
    def collect_samples(self):
        spike_path = osp.join(self.cfg['data']['root'], 'test', 'spike', 'val', 'interp_{:d}_alpha_{:.2f}'.format(self.cfg['data']['interp'], self.cfg['data']['alpha']))
        image_path = osp.join(self.cfg['data']['root'], 'test', 'imgs', 'val', 'val_orig')
        scene_list = sorted(os.listdir(spike_path))
        samples = []
